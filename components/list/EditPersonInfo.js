...
export default class EditPersonInfo extends Component {
    ...
    saveHandler() {
        ...
        db.updatePerson(this.state.tableName, this.state.index_no, this.state.name, this.state.phone).then((result) => {
            Alert.alert('Success', 'Edited Successfully', [{
                text: 'Ok', onPress: () => this.props.navigation.navigate('TableContent',
                    { tableName: this.state.tableName }), style: 'default'
            }]);
        }).catch((err) => {
            console.log(err)
            this.setState({
        ...
        return (
            <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps='handled'>
                <Surface style={styles.container}>
                    <Text style={styles.inputMessage}>Edit Details </Text>
